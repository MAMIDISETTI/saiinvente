...
class App extends Component {
  ...
  onChangeSearcInput = event => {
    this.setState({searchInput: event.target.value})
  }

  render() {
    const {searchInput} = this.state
    const searchResults = urlVideo.filter(each =>
    ...
    return (
      <>
        <div className="app-container">
          <input
            type="search"
            onChange={this.onChangeSearcInput}
            value={searchInput}
            className="input"
        ...
        <ul className="un-order">
          {searchResults.map(one => (
            <VideoPlayer urlVideos={one} key={one.title} />
          ))}
        </ul>
      </>
    )
